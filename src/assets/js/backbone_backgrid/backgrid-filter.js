...
(function (root, factory) {
  ...
  */
  var ServerSideFilter = Backgrid.Extension.ServerSideFilter = Backbone.View.extend({
    ...
    // template: _.template('<span id="detail"></span><input class="form-control" id="search" style="width: 300px;" type="search" <% if (placeholder) { %> placeholder="<%- placeholder %>" <% } %> name="<%- name %>" <% if (value) { %> value="<%- value %>" <% } %>/>', null, {variable: null}),
    template: _.template('<div id="wrap"> <form action="" autocomplete="on"> <input id="search" name="search" type="search"  <% if (placeholder) { %> placeholder="<%- placeholder %>" <% } %> name="<%- name %>" <% if (value) { %> value="<%- value %>" <% } %>/><input id="search_submit" value="Rechercher" type="submit"> </form> </div> ', null, {variable: null}),
